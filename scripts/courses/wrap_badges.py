            ...
            while i < len(lines) and "</div>" not in lines[i]:
                ...
                line = re.sub(
                    ...
                    lambda m: m.group(0).replace(
                        m.group(0), re.sub(r"([\u4e00-\u9fff]+)", lambda c: urllib.parse.quote(c.group()), m.group(0))
                    ),
                    line,
                )
...
示例：
  ...
  %(prog)s -c "custom-class" file.md  # 使用自定义类名
    ...
    # 添加参数
    parser.add_argument("files", nargs="+", type=Path, help="要处理的 Markdown 文件路径")
    parser.add_argument("-b", "--backup", action="store_true", help="处理前创建备份文件（.md.bak）")
    parser.add_argument(
        "-c",
        "--classes",
